...
import { getBakingItems, getRecipe, getDependencies, getResources, getCategories } from "@/services/api";
...
export function KitchenPage() {
  ...
  // Charger les catégories filtrées pour Baking
  useEffect(() => {
    getCategories("Baking").then(setCategories).catch(console.error);
  }, []);
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      try {
        const items = await getBakingItems(query, category || undefined);
        setResults(items);
      } catch (err) {
        console.error(err);
      } finally {
    ...
    <div className="flex flex-col gap-6 lg:grid lg:grid-cols-2">
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Items obtenables par cuisson au four
        </p>
